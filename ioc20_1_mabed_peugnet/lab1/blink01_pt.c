...
main ( int argc, char **argv )
{
    ...
    printf("test12 ouessheu \n");
    struct thread_args args1;
    args1.half_period = half_period;
    args1.led = GPIO_LED0;

    struct thread_args args2;
    args2.half_period = half_period;
    args2.led = GPIO_LED1;

    pthread_t thread1;
    pthread_t thread2;
    printf("test2");
    if (pthread_create(&thread1, NULL, blink, &args1)) {
        perror("pthread_create");
        return EXIT_FAILURE;
    }
    if (pthread_create(&thread2, NULL, blink, &args2)) {
        perror("pthread_create");
        return EXIT_FAILURE;
    }
